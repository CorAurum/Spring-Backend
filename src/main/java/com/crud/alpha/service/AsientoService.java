...
public class AsientoService {
    ...
    public List<Asiento> listarPorNroCoche(int nroCoche) {
        return asientoRepository.findByNroCoche_NroCoche(nroCoche);
    }
    ...
    public void crearAsientosParaOmnibus(int nroCoche, int cantidadAsientos) throws EntityNotFoundException {
        ...
        Omnibus omnibus = omnibusService.buscarOmnibusPorNroCoche(nroCoche)
                .orElseThrow(() -> new EntityNotFoundException("No se encontró el omnibus con nroCoche: " + nroCoche));

        // Crear asientos y asociarlos al omnibus
        for (int i = 1; i <= cantidadAsientos; i++) {
            Asiento asiento = new Asiento();
            asiento.setNroAsiento(i);
            asiento.setNroCoche(omnibus);
